...
public:
	vector<vector<string>> findLadders(string start, string end, unordered_set<string> &dict) {
		...
		while (!current.empty() && !found){
			...
			for (auto itr = current.begin(); itr != current.end(); itr++){
				string str = *itr;
				vector< string > strNeighbors = findNeighbors(str, visited, dict);
				for (int i = 0; i < strNeighbors.size(); i++){
					string &str_n = strNeighbors[i];
		...
		if (found){ 
			vector<string> path;
			BuildPath(parent, start, end, path, results);
		}
...
private:
	...
	vector< string > findNeighbors(string str, unordered_set<string> &visited, unordered_set<string> &dict)
	{
		int i, j;
		vector<string> strNeighbors;
		...
		) // DFS scheme
	{
		...
		for (int i = 0; i < pr.size(); i++){
			BuildPath(parent, start, pr[i], path, results);
		}
		path.pop_back();
	}
